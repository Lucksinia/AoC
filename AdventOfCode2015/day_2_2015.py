...
def meashure_wrapper(dim: list) -> int:
    """measure wrapper size for each individual present

    :param list dim: dimentions of present as a list
    :return int: result size
    """
    sides = [dim[0] * dim[1], dim[1] * dim[2], dim[0] * dim[2]]
    result = (2 * sides[0]) + (2 * sides[1]) + (2 * sides[2]) + min(sides)
    return result
...
with open("AdventOfCode2015/2.txt", "r") as f:
    ...
    for str in meashurments:
        str = str.strip()
        str = str.split("x")
        dim = [eval(i) for i in str]  # evaluate if integer
        res.append(meashure_wrapper(dim))
    print(f"total square feet of wrapping paper they sould order: {sum(res)}")
...
def meashure_ribbon(dim: list) -> int:
    """measure ribbon size for each individual present

    :param list dim: dimentions of present as a list
    :return int: result size
    """
    min_side = 2 * min(dim[0] + dim[1], dim[1] + dim[2], dim[0] + dim[2])
    result = min_side + (dim[0] * dim[1] * dim[2])
    return result
...
for str in meashurments:
    str = str.strip()
    str = str.split("x")
    dim = [eval(i) for i in str]
    res.append(meashure_ribbon(dim))
print(f"total square feet of ribbon they sould order: {sum(res)}")
